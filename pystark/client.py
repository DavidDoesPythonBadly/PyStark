...
class Stark(Client, Mechanism):
    ...
    def activate(self):
        ...
        try:
            ...
            self.stop()
            logger.info(
                "Bot has stopped working. For issues, visit <https://t.me/StarkBotsChat>"
            )
        ...
        for t in tables:
            if t.__tablename__ == name:
                ...
                if name == "users":
                    ...
                    db = Database()
                    if "lang" not in asyncio.get_event_loop().run_until_complete(
                        db.columns("users")
                    ):
                        # Temporary Way
    ...
    async def send(self, *args, **kwargs):
        """Send a request to Telegram servers.

        This method is a wrapper around `pyrogram.Client.send` method.
        """
